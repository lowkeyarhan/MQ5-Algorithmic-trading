...
public:
   ...
   void UpdateDashboard(
      ...
      string gmtStr = "GMT: " + IntegerToString(dt.hour, 2, '0') + ":" + IntegerToString(dt.min, 2, '0');
      CreateLabel("time", gmtStr, x+160, y + (row-1)*dy, clrDimGray, 8);
      ...
      string hiStr = "---";
      string loStr = "---";
      if(asiaHigh > 0) hiStr = DoubleToString(asiaHigh, asiaDigits);
      if(asiaLow > 0 && asiaLow < 9999) loStr = DoubleToString(asiaLow, asiaDigits);
      ...
      row++;
      CreateLabel("bal", "Balance  : $" + DoubleToString(balance, 2), x, y + row*dy, clrSilver);
      row++;

      color pnlClr = (dayPnL >= 0) ? m_colorBull : m_colorBear;
      string pnlSign = (dayPnLPct >= 0) ? "+" : "";
      string pnlStr = "Day P&L  : " + pnlSign + DoubleToString(dayPnLPct, 2) + "% ($" + DoubleToString(dayPnL, 2) + ")";
      CreateLabel("pnl", pnlStr, x, y + row*dy, pnlClr);
      row++;
      CreateLabel("trades", "Trades   : " + IntegerToString(tradesToday) + " today", x, y + row*dy, clrSilver);
      row++;
      ...
      string streakStr = "Streak   : " + IntegerToString(consecWins) + "W / "
                       + IntegerToString(consecLosses) + "L  x"
                       + DoubleToString(sizeMult, 1);
      CreateLabel("streak", streakStr, x, y + row*dy, clrSilver);
      row++;

      CreateLabel("sep4", "----------------------------", x, y + row*dy, clrDimGray, 8);
      row++;

      color dirClr = clrDimGray;
      string dirStr = "none";
      if(lastDir == SIGNAL_BUY)  { dirStr = "BUY";  dirClr = m_colorBull; }
      if(lastDir == SIGNAL_SELL) { dirStr = "SELL"; dirClr = m_colorBear; }
