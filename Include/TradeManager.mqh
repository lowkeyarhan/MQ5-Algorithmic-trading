//+------------------------------------------------------------------+
//|                                               TradeManager.mqh  |
//|              Trade Execution for SMC Scalper EA v4.0            |
//+------------------------------------------------------------------+
...
class CTradeManager {
    ...
    bool PlaceBuyLimit(string symbol, double entry, double sl, double tp, double lot) {
        ...
        if(ask <= entry + tolerance) {
            ...
            Print("[Trade] BUY MARKET filled: ", symbol,
        ...
        if(!ok) {
            ...
            if(retcode == 10016 || retcode == 10015 || retcode == 10014) {
                Print("[Trade] Trying BUY MARKET fallback: ", symbol);
                bool fb = m_trade.Buy(lot, symbol, 0, sl, tp, "SMC_BUY_FB");
                if(fb) {
                    Print("[Trade] BUY MARKET fallback OK: ", symbol);
                    return true;
                }
            }
            return false;
        ...
        Print("[Trade] BUY LIMIT placed: ", symbol,
    ...
    bool PlaceSellLimit(string symbol, double entry, double sl, double tp, double lot) {
        ...
        if(bid >= entry - tolerance) {
            ...
            Print("[Trade] SELL MARKET filled: ", symbol,
            " Lot = ", DoubleToString(lot, 2));
            return true;
        }
        ...
        if(!ok) {
            ...
            " ret = ", IntegerToString(retcode));
            if(retcode == 10016 || retcode == 10015 || retcode == 10014) {
                Print("[Trade] Trying SELL MARKET fallback: ", symbol);
                bool fb = m_trade.Sell(lot, symbol, 0, sl, tp, "SMC_SELL_FB");
                if(fb) {
                    Print("[Trade] SELL MARKET fallback OK: ", symbol);
                    return true;
                }
            }
            return false;
        ...
        AddPending(symbol, m_trade.ResultOrder(), entry, sl, tp);
        Print("[Trade] SELL LIMIT placed: ", symbol,
        " @", DoubleToString(entry, digits),
        " SL = ", DoubleToString(sl, digits),
        " TP = ", DoubleToString(tp, digits),
        " Lot = ", DoubleToString(lot, 2));
    ...
    void ManagePositions() {
        ...
        for(int i = total - 1; i >= 0; i--) {
         ...
         // BREAKEVEN at 1R
            if(m_useBreakeven && rMultiple >= 1.0) {
                ...
                if(!alreadyBE) {
                    ...
                    if(modified)
                    Print("[Trade] BE set: ", symbol, " #", IntegerToString((long)ticket),
                    " SL = ", DoubleToString(newSL, digits));
                }
            }

         // PARTIAL CLOSE 50% at 1.5R
            if(m_usePartial && rMultiple >= 1.5 && !IsPartialDone(ticket)) {
                double minLot = SymbolInfoDouble(symbol, SYMBOL_VOLUME_MIN);
                double lotStep = SymbolInfoDouble(symbol, SYMBOL_VOLUME_STEP);
                if(lots > minLot * 1.5) {
                    double closeLots = MathFloor((lots * 0.5) / lotStep) * lotStep;
                    ...
                    if(closeLots >= minLot && closeLots < lots) {
                        ...
                        if(closed) {
                            ...
                            Print("[Trade] PARTIAL 50 % % at 1.5R: ", symbol,
         ...
         // TRAIL at 1R distance once past 2R
            if(m_useTrail && rMultiple >= 2.0) {
                ...
                if(isBuy) {
                    ...
                    if(newSL > sl + point) {
                        ...
                        if(trailed)
                        Print("[Trade] Trail: ", symbol, " SL = ", DoubleToString(newSL, digits));
                    }
                } else {
                    double newSL = NormalizeDouble(currentPrice + trailDist, digits);
                    if(newSL < sl - point) {
    ...
    void CloseAll() {
        ...
        for(i = PositionsTotal() - 1; i >= 0; i--) {
            ...
            if(IsOurPosition(ticket)) {
                ...
                if(closed) Print("[Trade] Closed position #", IntegerToString((long)ticket));
            }
        }
        for(i = OrdersTotal() - 1; i >= 0; i--) {
            ...
            if(OrderGetInteger(ORDER_MAGIC) == MAGIC_NUMBER) {
                ...
                if(deleted) Print("[Trade] Deleted order #", IntegerToString((long)ticket));
            }
        }
    }
